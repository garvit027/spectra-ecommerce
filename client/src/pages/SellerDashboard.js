...
const ENDPOINTS = {
  ...
  createProduct: `${API_URL}/api/seller/products`,
  vacationToggle: `${API_URL}/api/users/vacation`,
};
...
export default function SellerDashboard() {
  ...
  const [productForm, setProductForm] = useState({
    ...
    imageFile: null, // NEW: for computer upload
  });
  ...
  // vacation toggle
  const toggleVacation = async (status) => {
    try {
      const res = await fetch(ENDPOINTS.vacationToggle, {
        method: "PATCH",
        headers: { "Content-Type": "application/json", ...authHeader },
        body: JSON.stringify({ onVacation: status }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to toggle vacation mode");
      
      // Update local state
      setAvailability(prev => ({ ...prev, paused: status }));
      
      // Update user in localStorage
      const updatedUser = { ...user, onVacation: status };
      localStorage.setItem("user", JSON.stringify(updatedUser));
      
      alert(`Store is now ${status ? "on Holiday (Products hidden)" : "Active (Products visible)"}`);
    } catch (e) {
      alert(e.message);
    }
  };

  // Sync initial vacation status from user object
  useEffect(() => {
    if (user && user.onVacation !== undefined) {
      setAvailability(prev => ({ ...prev, paused: !!user.onVacation }));
    }
  }, [user]);
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow">
            ...
            <div className="h-48 relative flex items-end gap-2 pt-6">
              {/* Y-axis line */}
              ...
              {salesData.length > 0 ? (
                salesData.map((d, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center justify-end h-full">
                    <div
                      className="w-full bg-purple-600 rounded-t hover:bg-purple-700 transition-colors cursor-help"
                      style={{ height: `${Math.max(6, (d.sales || 0) / 20)}px` }}
                      title={`${d.name}: ₹${Math.round(d.sales || 0)}`}
                    />
                    <span className="text-[10px] text-gray-400 mt-1 truncate w-full text-center">{d.name}</span>
                  </div>
                ))
              ) : (
                <div className="w-full h-full flex flex-col items-center justify-center text-gray-400 bg-gray-50/50 rounded-lg border-2 border-dashed border-gray-100 z-10">
                   <TrendingUp size={32} className="mb-2 opacity-30" />
                   <p className="text-sm font-medium">No sales data available</p>
                   <p className="text-xs opacity-60">Complete orders to see your growth</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-gray-900 flex items-center gap-2">
                <Shield size={20} className="text-purple-600" /> Holiday Mode
              </h3>
              <div 
                onClick={() => toggleVacation(!availability.paused)}
                className={`w-12 h-6 rounded-full p-1 cursor-pointer transition-all duration-300 ${availability.paused ? "bg-red-500" : "bg-green-500"}`}
              >
                <div className={`w-4 h-4 bg-white rounded-full shadow-sm transition-transform duration-300 ${availability.paused ? "translate-x-6" : "translate-x-0"}`} />
              </div>
            </div>

            <div className={`p-4 rounded-xl border transition-colors ${availability.paused ? "bg-red-50 border-red-100" : "bg-green-50 border-green-100"}`}>
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className={`font-bold ${availability.paused ? "text-red-700" : "text-green-700"}`}>
                  {availability.paused ? "Store is Hidden" : "Store is Live"}
                </span>
              </div>
              <p className="text-xs text-gray-600 leading-relaxed">
                {availability.paused 
                  ? "Your products are currently hidden from buyers. Turn off holiday mode to resume sales." 
                  : "Your products are visible and ready for orders. Switch to holiday mode to take a break."}
              </p>
            </div>

            {/* Additional availability controls (Optional) */}
            <div className="mt-4">
              <div className="text-xs font-bold text-gray-400 uppercase mb-2">Detailed Settings</div>
              <div className="grid grid-cols-3 gap-2">
                {["normal", "holiday", "vacation"].map(m => (
                  <button
                    key={m}
                    onClick={() => setAvailability(a => ({ ...a, mode: m }))}
                    className={`px-2 py-1.5 text-xs rounded border transition-colors ${availability.mode === m ? "bg-purple-600 text-white border-purple-600" : "bg-white text-gray-600 hover:bg-gray-50"}`}
                  >
            ...
            {availability.mode === "vacation" && (
              <div className="mt-3 grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="date"
                    className="border rounded px-2 py-1 text-xs w-full outline-none focus:border-purple-300"
            ...
            <button onClick={saveAvailability} className="mt-5 w-full bg-purple-600 text-white py-2 rounded-xl text-sm font-bold hover:bg-purple-700 transition-all shadow-md active:scale-95">
              Save Status
            </button>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(visibleProducts || []).map((p) => (
                <div key={p._id || p.id} className="flex gap-3 items-center border rounded p-3">
                  <img
                    src={p.images?.[0]?.startsWith('http') ? p.images[0] : (p.images?.[0] ? `${API_URL}/${p.images[0].startsWith('/') ? p.images[0].substring(1) : p.images[0]}` : "https://via.placeholder.com/90")}
                    alt={p.name}
                    className="w-20 h-20 object-cover rounded shadow-sm border border-gray-100"
                    onError={(e) => { e.target.onerror = null; e.target.src = 'https://via.placeholder.com/90'; }}
                  />
...
function ProductModal({
  ...
  const [uploadMode, setUploadMode] = React.useState("file"); // "file" | "url"
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex bg-gray-100 rounded-lg p-0.5">
                  <button 
                    ...
                    className={`px-3 py-1 text-xs rounded-md transition-all ${uploadMode === "file" ? "bg-white shadow-sm text-purple-700" : "text-gray-500"}`}
                  >
                    Computer
                  </button>
                  <button 
                    onClick={() => setUploadMode("url")}
                    className={`px-3 py-1 text-xs rounded-md transition-all ${uploadMode === "url" ? "bg-white shadow-sm text-purple-700" : "text-gray-500"}`}
                  >
                    URL Link
                  </button>
                </div>
              ...
              <div className="mt-2 space-y-2">
                ...
                {(form.images || [""]).map((url, idx) => (
                  <div key={idx} className="flex gap-2">
                    <input
                      type="text"
                      className="border rounded-lg px-3 py-2 flex-1 text-sm focus:ring-2 focus:ring-purple-200 outline-none transition-all"
